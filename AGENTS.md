...
**The mutation must land on a line the test file does not mention textually.** Mutating the string the test reads (`alignItems`, `contain`, a class name) only proves the test reads that string: red is guaranteed and worth nothing. If no such mutation exists, the test observes the fix rather than the behaviour — rewrite the test. This one is settled with `grep`, not judgement, and the agent that wrote both the test and the fix is exactly the one who cannot see it by feel.
