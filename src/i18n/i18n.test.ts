...
describe('Initial velocity entry labels', () => {
  beforeEach(() => setLang('pt-BR'))

  it('consumes pt-BR semantics for the mode and magnitude/angle fields', () => {
    const mode = t('properties.velocityMode').toLowerCase()
    expect(mode).toContain('modo')
    expect(mode).toContain('v₀')
    const cartesian = t('properties.velocityCartesian').toLowerCase()
    expect(cartesian).toContain('cartesiana')
    expect(cartesian).toContain('v₀')
    const polar = t('properties.velocityPolar').toLowerCase()
    expect(polar).toContain('polar')
    expect(polar).toContain('v₀')
    const magnitude = t('properties.v0Magnitude').toLowerCase()
    expect(magnitude).toContain('módulo')
    expect(magnitude).toContain('m/s')
    const angle = t('properties.v0Angle').toLowerCase()
    expect(angle).toContain('ângulo')
    expect(angle).toContain('°')
  })

  it('consumes EN semantics for the mode and magnitude/angle fields', () => {
    setLang('en')
    const mode = t('properties.velocityMode').toLowerCase()
    expect(mode).toContain('mode')
    expect(mode).toContain('v₀')
    const cartesian = t('properties.velocityCartesian').toLowerCase()
    expect(cartesian).toContain('cartesian')
    expect(cartesian).toContain('v₀')
    const polar = t('properties.velocityPolar').toLowerCase()
    expect(polar).toContain('polar')
    expect(polar).toContain('v₀')
    const magnitude = t('properties.v0Magnitude').toLowerCase()
    expect(magnitude).toContain('magnitude')
    expect(magnitude).toContain('m/s')
    const angle = t('properties.v0Angle').toLowerCase()
    expect(angle).toContain('angle')
    expect(angle).toContain('°')
  })
})
