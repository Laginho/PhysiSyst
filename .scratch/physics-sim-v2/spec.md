...
- **Good test = external behavior at a public seam.** No implementation details. The React/rAF adapter remains pinned through pure mirrors by default; a narrowly scoped DOM test is allowed when the behavior exists only at the adapter seam, as with the body-panel disclosure state approved for ticket 09.
